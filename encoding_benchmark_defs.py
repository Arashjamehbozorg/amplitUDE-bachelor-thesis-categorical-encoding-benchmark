...
BASE_DIR = Path(__file__).resolve().parent          # .../code/Boruta_Main_Amplitude
RESULTS_DIR = BASE_DIR / "results"                  # .../code/Boruta_Main_Amplitude/results
RESULTS_DIR.mkdir(parents=True, exist_ok=True)


# Silence xgboost glibc future warning
warnings.filterwarnings(
    "ignore",
    category=FutureWarning,
    module="xgboost.core"
)
# Silence boruta’s np.bool future warning spam
warnings.filterwarnings(
    "ignore",
    category=FutureWarning,
    module="boruta.boruta_py"
)
...
class EncodingBenchmark:
    ...
    def evaluate_combination(self, X, y, encoder_name, algorithm_name,
        ...
        try:
            ...
            rmse = np.sqrt(-cross_val_score(pipe, X, y, cv=cv, scoring="neg_mean_squared_error", n_jobs=1))
    ...
    def run_scenarios(self, X, y, cat_columns, num_columns, scenario_id=None):
        results = []
    ...
    # Scenario 1: No encoding, no Boruta
        if scenario_id is None or scenario_id == 0:
    ...
    # Scenario 2: Encoding, no Boruta
        if scenario_id is None or scenario_id == 1:    
    ...
    # Scenario 3: Boruta, no encoding
        if scenario_id is None or scenario_id == 2:    
    ...
    # Scenario 4: Boruta + encoding
        if scenario_id is None or scenario_id == 3:
        ...
        if not results:
            raise ValueError(f"No scenarios were run. Check scenario_id={scenario_id} (valid: 0–3 or None).")
    ...
    def visualize_results(self, results_df, dataset_name='dataset'):
        ...
        out = RESULTS_DIR / f"{dataset_name}_four_scenario_comparison_RMSE_R2.png"
...
def main(scenario_id =None):
    ...
    for dataset_name, loader in DATASETS_TO_RUN.items():
        ...
        results = bench.run_scenarios(X, y, cat_columns, num_columns, scenario_id=scenario_id)

        print("\n" + "📊 ANALYSIS".center(70, "-"))
        bench.analyze_results(results)

        print("\n" + "📈 VISUALIZATION".center(70, "-"))
        # add scenario suffix to plot name for clarity
        ds_name_for_plot = (
            f"{dataset_name}_scenario_{scenario_id}"
            if scenario_id is not None else dataset_name
        )
        bench.visualize_results(results, dataset_name=ds_name_for_plot)

        # avoid 4 jobs writing to the SAME csv
        if scenario_id is None:
            out_csv = RESULTS_DIR / f"{dataset_name}_benchmark_results.csv"
        else:
            out_csv = RESULTS_DIR / f"{dataset_name}_scenario_{scenario_id}_benchmark_results.csv"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="0–3 → run only that scenario; omit to run all scenarios."
    )
